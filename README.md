...
16. **tour-jet-ski-agadir-new.html** - Jet ski adventure
...
2. Run `npm run setup` to install Node.js dependencies and prepare the Python virtual environment
3. Start a local dev server with `npm run dev`
4. Open `http://127.0.0.1:3000` in your browser
5. Optional: use `npm run serve` if you want a simple static server without live reload
6. For production deployment, upload to any web hosting service
...
- `npm run setup` installs npm packages and creates `.venv`
- `npm run dev` starts a live-reload server on port 3000
- `npm run dev:public` exposes the live-reload server on your LAN
